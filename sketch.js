...
function draw() {
    ...
    if (!noise_active && mouse_active && adjustedMouseX >= padding && adjustedMouseX <= width - padding && 
        adjustedMouseY >= padding && adjustedMouseY <= height - padding) {
        mouseInfluence = true;
    }
    ...
    for (let y = 0; y < img.height; y += cellSize) {
        ...
        for (let x = 0; x < img.width; x += cellSize) {
            ...
            if (mouseInfluence) {
                ...
                let dx = adjustedMouseX - arrowX;
                let dy = adjustedMouseY - arrowY;
